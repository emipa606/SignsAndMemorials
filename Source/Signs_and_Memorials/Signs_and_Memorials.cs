...
namespace SaM
{
    ...
    public class SaM_ModSettings : ModSettings
    {
        ...
        {
            Scribe_Values.Look(ref editOnBuild, "edit_on_build", false, false);
            Scribe_Values.Look(ref pauseGameOnEdit, "pause_game_on_edit", false, false);
        }
    }
    ...
    public class Building_Memorial_Base : Building
    {
        ...
        public override void SpawnSetup(Map map, bool respawningAfterLoad)
        {
            ...
            if (SaM_Mod.settings?.editOnBuild ?? false)
            {
                ...
                ((MainTabWindow_Inspect)MainButtonDefOf.Inspect.TabWindow).OpenTabType = GetInspectTabs().OfType<ITab_View>().First().GetType();
            }
        }
    }
    ...
    public class ITab_View : ITab
    {
        ...
        {
            base.TabUpdate();
            if (tabThingID != base.SelThing.GetUniqueLoadID())
            {
                text = ((Building_Memorial_Base)SelThing).Text;
                tabThingID = base.SelThing.GetUniqueLoadID();
            }
        }
        ...
        protected override void FillTab()
        {
            ...
            Rect rectText = new Rect(20, 20, size.x - 40, size.y - 75);
            Rect rectButton = new Rect(20, size.y - 50, size.x - 40, 30);
            Rect rectCancel = new Rect(rectButton.x, rectButton.y, (rectButton.width - 10) / 2, rectButton.height);
            Rect rectSave = new Rect((size.x + 10) / 2, rectButton.y, (rectButton.width - 10) / 2, rectButton.height);
